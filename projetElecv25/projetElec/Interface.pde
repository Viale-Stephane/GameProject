class Interface {
  boolean inGame=false, firstScreen=true, ecranTitre=false, setUsername=false, load=false, credit=false, leaderboard=false, firstLoad=true, firstLeaderboard=true, save=false, pause=false;
  int line=1, column=1;
  ...
  void credits() {//affichage visuel de la page des crédits
    ...
    if (millis()-creditTime<=5000) {//change de page au bout de 5 secondes
      ...
      textSize(11);
      for (int i=1; i<=12; i++) {
        text("level "+i+" : Vincent Coppé", 50, 60+40*i);
        text("level "+(i+13)+" : Vincent Coppé", 350, 60+40*i);
      }
    ...
    if (interfaces.ecranTitre==true) {
      ...
      if (key==ACTION) {
        if (line==1 && column==1 && actualPage==1) {
          ...
          image(menuEmpty, 0, 0);
          text("Connectez-vous à la PI en SSH", 200, 100);
          text("Identifiant :pi", 200, 200);
          text("Mot de passe :colorpanic", 300, 200);
          //interfaces.inGame=true;
        } else {
      ...
      if (key==ACTION) {
        if (line==1 && column==1 && actualPage==1) {
          ...
          actualPage++;
        } else if (line==8) {//quitte l'interface de sauvegarde
