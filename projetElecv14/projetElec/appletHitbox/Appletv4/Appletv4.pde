...
void printImage() {
  ...
  for (int i=0; i<128; i++) {
    for (int j=0; j<38; j++) {
      ...
      } else if (i%8==0 && j%4==0 && j<35 && i<121 && type[i+128*j].equals("door")) {
        for (int k=1; k<8; k++) {
          for (int l=0; l<4; l++) {
            type[i+k+128*(j+l)]="end"+lvl.substring(3);
          }
        }
        type[i+128*(j+1)]="end"+lvl.substring(3);
        type[i+128*(j+2)]="end"+lvl.substring(3);
        type[i+128*(j+3)]="end"+lvl.substring(3);
      }
      ...
      if (type[i+128*j].equals("emptyspikeDarkBlueBottom") || type[i+128*j]=="emptyspikeDarkBlueBottom") {
        ...
        img=wallGreen[19];
      } else if (type[i+128*j].equals("door") ||type[i+128*j]=="door") {
        img=door;
      }
      if (type[i+128*j].equals("BJump") || type[i+128*j]=="BJump") {
        ...
        if ((!type[i+128*j].equals("wall") && !type[i+128*j].equals("wallNoClip") && type[i+128*j].startsWith("wall")) || (!type[i+128*j].equals("spike") && type[i+128*j].startsWith("spike")) || (!type[i+128*j].equals("spike") && type[i+128*j].startsWith("emptyspike")) ||type[i+128*j].equals("door")) {
