...
void draw() {
  if (interfaces.ecranTitre==true) {
    ...
    if (((millis()-initialTime)-timeStopped)%1000<=16) {
      second++;
    }
    if (second==60) {
      minute++;
    ...
    fill(126, 56, 186);
    text("TIMER:"+hour+":"+minute+":"+second+":"+(((millis()-initialTime)-timeStopped+1000)-(second*1000)-(minute*60000)-(hour*3600000)), 640, 50);//880
    text("MORTS:"+hero.nbMort, 50, 50);
    text("COINS: "+bonusPoints.nbPoints, 400, 50);
    fill(255, 255, 255);
    hero.contactSpike();
    hero.contactEnd();
  }
...
void keyReleased() {
  if (interfaces.setUsername==false && interfaces.load==false && interfaces.ecranTitre==false) {
...
void keyPressed() {
  if (interfaces.ecranTitre==false && interfaces.setUsername==false && interfaces.load==false) {
